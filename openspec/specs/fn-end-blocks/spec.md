# Capability: Function and block syntax (`fun` / `end`)

## Purpose

Lucky uses `fun` for function declarations, `end` to close blocks, optional `then` after conditions, a single `elseif` keyword, and short-form anonymous functions. Braces are not block delimiters.

## Requirements

### Requirement: `fun` keyword introduces functions

The lexer SHALL recognize `fun` as a reserved keyword and MUST NOT tokenize it as an identifier. The parser SHALL use `fun` to begin both named and anonymous function declarations.

#### Scenario: fun is a reserved keyword

- **WHEN** a program contains `fun = 1`
- **THEN** the parser raises a syntax error

#### Scenario: named function uses fun keyword

- **WHEN** a program contains `fun add(a, b)\n  return a + b\nend`
...
#### Scenario: anonymous function uses fun keyword

- **WHEN** a program contains `x = fun(a, b)\n  return a + b\nend`
...
- **WHEN** a program contains `fun foo() { return 1 }`
- **THEN** the parser raises a syntax error

#### Scenario: end closes a function body

- **WHEN** a program contains `fun foo()\n  return 1\nend`
...
The short form is restricted to anonymous functions (no name after `fun`). Named functions always use the full form.

#### Scenario: short-form lambda with single expression

- **WHEN** a program contains `fun(x) x * 2`
...
- **WHEN** a program runs `f = fun(x) x * 2` then calls `f(3)`
- **THEN** the call returns `6`

#### Scenario: short-form lambda as callback argument

- **WHEN** a program passes `fun(x) x * 2` as an argument to a function
...
- **WHEN** a program contains `fun(a, b) a + b`
...
- **WHEN** a program contains `fun(x) x * 2 end`
...
- **WHEN** a program contains `fun add(a, b) a + b`
...
- **WHEN** a program contains `fun(x) return x`
...
- **WHEN** a program defines `fun foo()\n  1 + 2\nend` and calls `foo()`
...
- **WHEN** a program defines `fun classify(n)\n  if n < 0\n    return -1\n  end\n  return n * 2\nend` and calls `classify(-5)`
