# Capability: Variable declarations (`let`)

## Purpose

Bindings are introduced with `let`. Simple and compound assignment require an existing writable binding and update the nearest enclosing definition.
...
- **WHEN** the program runs `let x = 1`, defines `fun foo(); let x = 2; return x; end`, calls `foo()`, then evaluates `x`
...
- **WHEN** the program defines `fun foo(); x = 1; end` with no declaration of `x` in any enclosing writable scope, then calls `foo()`
- **THEN** evaluation raises `NameError` naming `x`
...
- **WHEN** the program defines `fun make(); let n = 0; fun inc(); n = n + 1; end; inc(); inc(); return n; end` and calls `make()`
- **THEN** the call returns `2`

#### Scenario: nearest declaration wins during reassignment

- **WHEN** the program runs `let x = 1`, defines `fun foo(); let x = 5; x = 9; return x; end`, calls `foo()`, then evaluates outer `x`
