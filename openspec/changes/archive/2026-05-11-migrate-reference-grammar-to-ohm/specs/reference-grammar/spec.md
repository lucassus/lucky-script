...
`src/Lexer/Lexer.test.ts` MUST contain probe tests that characterize the Lucky Lexer's behavior on two known parity concerns surfaced by this change. The probe tests serve as a baseline for a follow-up change; **failures are acceptable in this milestone** if marked with a `TODO(grammar-ohm-followup)` code comment pointing to `openspec/changes/migrate-reference-grammar-to-ohm/`. This change MUST NOT modify the Lexer to make the probes pass.
